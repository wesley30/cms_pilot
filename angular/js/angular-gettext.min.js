...
angular.module('gettext').factory('gettextCatalog', [
  ...
  function (gettextPlurals, $http, $cacheFactory) {
    ...
    catalog = {
      ...
      prefixDebug: function (string) {
        if (this.debug && this.currentLanguage !== this.baseLanguage) {
          this.strings[this.currentLanguage][string] = [string];
      ...
      CreateJavascript: function(Lang){ 
          this.JavaScriptOutPut = "gettextCatalog.setStrings('"+Lang+"', { \n";
          
          var LangList = this.strings[this.currentLanguage];
          var Once = false;
          
          for (var key in LangList) {
            if (LangList.hasOwnProperty(key)) {
              //console.log(key + " -> " + LangList[key]);
              if(Once){
                this.JavaScriptOutPut += ",'"+key+"': ''\n";
              }else{ 
                this.JavaScriptOutPut += "'"+key+"': ''\n";
              }
      ...
      getString: function (string) {
        return this.getStringForm(string, 0) || this.prefixDebug(string);
      },
      getPlural: function (n, string, stringPlural) {
        var form = gettextPlurals(this.currentLanguage, n);
        return this.getStringForm(string, form) || this.prefixDebug(n === 1 ? string : stringPlural);
      },
